...
Data collection is possible with the following minimum criteria  
...
Collected data can be analyzed on PostgreSQL 16 or above, which supports modern analytical functions.


# Highlights
1. **Secure by Open :** Simple, Transparent, Fully auditable code.  
...
10. **Small, single file data dump** :  
   To generate the smallest possible file, which can be further compressed by `gzip` for easy transmission and storage, `pg_gather` avoids redundancy in the collected data as much as possible.
...
```
OR ALTERNATIVELY pipe to a compression utility to get a compressed output as follows:
```
...
<a name="notes">
## Notes:</a> 
   1. **Heroku** and similar DaaS hostings impose very high restrictions on collecting performance data. Queries on views like `pg_statistic` may produce errors during data collection, but these errors can be ignored.
   2. **MS Windows** users!, 
   ...
   ```
   3. **AWS Aurora** offers a "PostgreSQL-compatible" database. However, it is not a true PostgreSQL database, even though it looks like one. Therefore, you should do the following to the `gather.sql` script to replace any inapplicable lines with "NULL".
   ```
...
There could be requirements for collecting data continuously and repeatedly. `pg_gather` has a special lightweight mode for continuous data gathering, which is automatically enabled when it connects to the "template1" database. Please refer to detailed [documentation specific to continuous and repeated data collection](docs/continuous_collection.md).
...
NOTE: PostgreSQL version 16 or above is required to generate the analysis report.



# ANNEXURE 1 : Using PostgreSQL container and wrapper script
...
The script will spin up a Docker container, import the output of `gather.sql` (out.tsv) and generates an HTML report. This script expects at least a single argument: path to the `out.tsv`. 

There are two additional positional arguments: 
* Desired report name with path. 
* A flag to specify whether to keep the docker container. This flag allows usage of the container and data for further analysis.
...
Finished generating report in /tmp/out.tsv.html
```
...
However, this may not be the right timezone for a few users, especially when cloud hostings are used. The `pg_gather` allows the user to have a custom timezone by setting the environment variable `PG_GATHER_TIMEZONE` to override the default. For example,
```
