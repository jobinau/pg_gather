...
DROP TABLE IF EXISTS pg_gather, pg_get_activity, pg_get_class, pg_get_am, pg_get_confs, pg_get_file_confs, pg_get_db_role_confs, pg_get_db, pg_get_index,pg_get_tablespace, 
  ...
  pg_get_relstats, pg_get_toast, pg_get_statements, pg_get_bgwriter, pg_get_roles, pg_get_extension, pg_get_slots, pg_get_hba_rules, pg_get_ns, pg_gather_end, pg_get_prep_xacts;
...
CREATE UNLOGGED TABLE pg_get_locks(
  pid int,
  relation oid
);
...
CREATE UNLOGGED TABLE pg_get_relstats (
    relid         oid,
    attcount      integer,          -- columns with stats
    any_null      boolean,          -- any column has null_frac > 0
    tpl_data_size double precision -- sum((1-null_frac)*avg_width)
);
...
CREATE UNLOGGED TABLE pg_get_shmem(
 name text,
